      ...
      {showForm && (
        <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4 shadow-sm">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            <div>
              <label className="text-xs font-medium text-[#6B6B8A] mb-1 block">בן/בת זוג</label>
              <select value={newEntry.spouse} onChange={(e) => setNewEntry({ ...newEntry, spouse: e.target.value as 'spouse1' | 'spouse2' })} className={`${inputCls} cursor-pointer`}>
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-lavender-light text-[#4A4A60]">
              ...
              <th className="px-4 py-2.5 text-right font-semibold text-xs">בן/בת זוג</th>
