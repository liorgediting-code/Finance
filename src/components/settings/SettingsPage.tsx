...
export default function SettingsPage() {
  ...
  const [accName, setAccName] = useState(accountantName ?? '');
  const [accEmail, setAccEmail] = useState(accountantEmail ?? '');
  const [accPhone, setAccPhone] = useState(accountantPhone ?? '');
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session?.access_token) {
        setGmailError('לא מחובר — נסה להתחבר מחדש');
      ...
      .then((r) => r.json())
      .then((data: { error?: string; accessToken?: string; expiresAt?: string; gmailEmail?: string }) => {
        ...
        .catch((e: Error) => setGmailError(e.message))
    ...
    <div className="space-y-6 p-4 md:p-6 max-w-3xl mx-auto" dir="rtl">
      ...
      <SettingsSection title="שנה פעילה" accentColor="#C5CDB6">
        ...
        <input
          type="number"
          value={settings.year}
          onChange={(e) => updateSettings({ year: parseInt(e.target.value, 10) || 2026 })}
